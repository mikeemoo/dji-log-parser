...
util.inherits(DJIParser, EventEmitter);

DJIParser.prototype.parseFile = function(file) {
  var buffer = fs.readFileSync(file);
  return this.parseBuffer(buffer);
}

DJIParser.prototype.parseBuffer = function(buffer) {
  
  var dataLen = buffer.readInt32LE(0);
  ...
  while (offset < dataLen) {
    ...
    var tId = buffer[offset++];
    var type = types[tId];

    // second byte is packet length
    var length = buffer[offset++];

    var end = buffer[offset + length];
